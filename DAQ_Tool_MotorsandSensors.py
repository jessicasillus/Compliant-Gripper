#!/usr/bin/env python3
"""
ST3215-12V Motor Control + MPR121 Sensor Logger — Combined

Sensitivity improvements:
  - Live Sensitivity slider (1x–5x): amplifies the delta from baseline
    before interpolation, so gentle touches register more strongly.
  - Live Deadband slider (0–100): minimum delta required to register any
    signal; lower = catches lighter contacts.
  - Both sliders update in real time with no restart needed.
  - RECAL button / R key resets baselines from fresh data.

Motor close-setpoint:
  - Use M1 +15° / M1 -15° to jog to desired close position, then press
    "M1 Set↓" to save it.  "M1 Close" will then return to that position.
  - Same workflow for M2.
  - HOME still returns both motors to their original startup positions.

Sensor wiring (serial position → label):
  Position 1 → C3
  Position 2 → C2
  Position 3 → C1
  Position 4 → B3
...
Default ports (auto-tried on startup, prompted only if connection fails):
  ESP32  → COM4
  Motors → COM11
"""
...
MIN_UNLOADED_RAW = 500   # baseline sanity threshold
...
# ─────────────────────────────────────────────────────────────────────────────
# MotorState  —  supports a per-motor "close" setpoint
# ─────────────────────────────────────────────────────────────────────────────
...
class MotorState:
    ...
    def set_m1_close(self):
        ...
        if p is not None:
            ...
            self.status = "M1 SET CLOSE failed — couldn't read position"
        print(self.status)
    ...
    def set_m2_close(self):
        ...
        if p is not None:
            ...
            self.status = "M2 SET CLOSE failed — couldn't read position"
        print(self.status)

    def m1_close(self, time_ms=500):
        if self.close_pos['m1'] is None:
            self.status = "M1: no close setpoint — jog then press M1 Set↓"
    ...
    def m2_close(self, time_ms=500):
        if self.close_pos['m2'] is None:
            self.status = "M2: no close setpoint — jog then press M2 Set↓"
            print(self.status); return
...
# ─────────────────────────────────────────────────────────────────────────────
# SensorLogger — with live sensitivity + deadband overrides
# ─────────────────────────────────────────────────────────────────────────────
...
class SensorLogger:
    def __init__(self, max_points=150, calibration=None):
        ...
        # Sensor wiring: serial position (0-based) → sensor label
        # Position: 1→C3, 2→C2, 3→C1, 4→B3, 5→B2, 6→B1, 7→A3, 8→A2, 9→A1
        self.raw_map  = ['C3', 'C2', 'C1', 'B3', 'B2', 'B1', 'A3', 'A2', 'A1']
        self.save_dir = r"C:\Users\jessi\OneDrive - The Ohio State University\Jessica Sillus MS\VS Code\Sensor Programming"
    ...
    def _try_init_baselines(self):
        ...
        if not new:
            self.baseline_status = "Startup capture failed — all readings too low, retrying…"
    ...
    def set_baseline_now(self):
        if self._capture_baselines_from_current():
            ...
            self.baseline_status = "Set Baseline failed — readings look too low"
...
# ─────────────────────────────────────────────────────────────────────────────
# Port helpers — try default silently, fall back to prompt on failure
# ─────────────────────────────────────────────────────────────────────────────

def open_serial_port(default_port, baudrate, timeout, label):
    """
    Try the default port silently.
    If it fails, print a message and prompt the user once.
    Returns (serial_object, port_string) or (None, None) if skipped.
    """
    try:
...
def main():
    ...
    if ser is None:
        print("ESP32 is required — exiting.")
        return
    ...
    # =========================================================================
    # Figure layout
    # Row 0: sensor plot  |  sidebar
    # Row 1: note TextBox + sensitivity sliders (full width)
    # Row 2: motor buttons (only if motors connected)
    # =========================================================================
    ...
    # ── Two baseline buttons stacked at the bottom of the sidebar ─────────────
    ...
    # ── Note TextBox + Sensitivity sliders inside ax_ctrl ────────────────────
    fig.canvas.draw()
    ...
    ax_ctrl.text(
        0.0, 0.16,
        "Sensitivity: amplifies delta from baseline (higher = more response to light touch)\n"
        "Deadband: min change to register signal (lower = catches lighter contacts; 'auto' uses calibration default)",
