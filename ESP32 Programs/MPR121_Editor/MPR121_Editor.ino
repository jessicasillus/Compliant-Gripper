...
const int NUM_SENSORS = 9;  // (A1-A3, B1-B3, C1-C3)
...
void setup() {
  ...
  // Reconfig to match sensors in 60-500pF range
  ...
  // enable NUM_SENSORS electrodes (ELE0 through ELE8)
  cap.writeRegister(0x5E, 0x80 | NUM_SENSORS);

  // start smoothing buffer with real sensor readings for better avgs
...
void loop() {
  ...
  for (int sensor = 0; sensor < NUM_SENSORS; sensor++) {
    ...
    if (sensor < NUM_SENSORS - 1) {
      Serial.print(","); // print to terminal for python to use later
    }
  }
